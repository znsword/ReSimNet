...
class DrugModel(nn.Module):
    ...
    def init_lstm_h(self, batch_size):
        # 原实现：使用 Variable 并调用 cuda
        # return (Variable(torch.zeros(
        # 	self.lstm_layer*1, batch_size, self.drug_embed_dim)).cuda(),
        #         Variable(torch.zeros(
        # 	self.lstm_layer*1, batch_size, self.drug_embed_dim)).cuda())
        # 新实现：直接使用张量并迁移到设备
        h0 = torch.zeros(self.lstm_layer*1, batch_size, self.drug_embed_dim, device=self.device)
        c0 = torch.zeros(self.lstm_layer*1, batch_size, self.drug_embed_dim, device=self.device)
        return (h0, c0)
    ...
    def siamese_sequence(self, inputs, length):
        ...
        if not self.training:
            ...
            # 原实现：Variable(sort_idx).cuda()
            # c_embed = c_embed.index_select(0, Variable(sort_idx).cuda())
            # 新实现：直接迁移张量到设备
            c_embed = c_embed.index_select(0, sort_idx.to(self.device))
        ...
        if not self.training:
            ...
            # 原实现：Variable(unsort_idx).cuda()
            # outputs = hidden.index_select(0, Variable(unsort_idx).cuda())
            # 新实现：直接使用张量并迁移到设备
            outputs = hidden.index_select(0, unsort_idx.to(self.device))
        else:
            outputs = hidden
    ...
    def graph_conv(self, features, adjs):
        ...
        # 新实现：指定 dim，提高兼容性
        graph_conv = F.log_softmax(layer4_out, dim=-1)
    ...
    def distance_layer(self, vec1, vec2, distance='cos'):
        ...
        if self.binary:
            ...
            # 新实现：使用 torch.sigmoid（与 nn.BCELoss 配合）
